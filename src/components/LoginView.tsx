  ...
  const portalTabs = [
    {
      ...
      title: t.login.portalTabs.patient.title,
      sub: t.login.portalTabs.patient.sub,
      icon: User,
      color: 'rose',
      borderActive: 'border-rose-600 bg-rose-50/70 text-rose-900',
      badge: t.login.portalTabs.patient.badge
    },
    {
      id: 'doctor' as PortalType,
      title: t.login.portalTabs.doctor.title,
      sub: t.login.portalTabs.doctor.sub,
      icon: Stethoscope,
      color: 'indigo',
      borderActive: 'border-indigo-600 bg-indigo-50/70 text-indigo-900',
      badge: t.login.portalTabs.doctor.badge
    },
    {
      id: 'admin' as PortalType,
      title: t.login.portalTabs.admin.title,
      sub: t.login.portalTabs.admin.sub,
      icon: ShieldCheck,
      color: 'slate',
      borderActive: 'border-slate-800 bg-slate-100 text-slate-900',
      badge: t.login.portalTabs.admin.badge
    },
    {
      id: 'field' as PortalType,
      title: t.login.portalTabs.field.title,
      sub: t.login.portalTabs.field.sub,
      icon: Users,
      color: 'teal',
      borderActive: 'border-teal-600 bg-teal-50/70 text-teal-900',
      badge: t.login.portalTabs.field.badge
    }
  ];
  ...
  const keyDemoProfiles = [
    {
      ...
      tag: language === 'hi' ? 'उच्च जोखिम गर्भवती' : language === 'mr' ? 'उच्च जोखीम गर्भवती' : 'High-Risk ANC',
      icon: User,
      desc: language === 'hi' ? 'स्वतः ओटीपी (4821) दर्ज होगा और आभा लॉकर खुलेगा' : language === 'mr' ? 'आपोआप ओटीपी (4821) नोंदवून आभा लॉकर उघडेल' : 'Auto-fills OTP (4821) & opens ABHA Locker'
    },
    {
      profile: doctorUsers[0], // Dr. Rajesh Kulkarni
      roleLabel: t.roles.doctor,
      tag: language === 'hi' ? 'चांदूर प्राथमिक स्वास्थ्य केंद्र' : language === 'mr' ? 'चांदूर प्रा.आ. केंद्र' : 'Chandur PHC',
      icon: Stethoscope,
      desc: language === 'hi' ? 'ओपीडी ट्राइएज एवं टेलीकंसल्टेशन डेस्क' : language === 'mr' ? 'ओपीडी ट्राइएज व टेलिकन्सल्टेशन' : 'OPD Triage & Teleconsult Workstation'
    },
    {
      profile: ashaUsers[0], // Sunita Bai
      roleLabel: t.roles.asha,
      tag: language === 'hi' ? 'रामपुर गाँव' : language === 'mr' ? 'रामपूर गाव' : 'Rampur Village',
      icon: Users,
      desc: language === 'hi' ? 'ऑफ़लाइन फील्ड स्क्रीनिंग व सर्वे' : language === 'mr' ? 'ऑफलाइन तपासणी व सर्वेक्षण' : 'Offline Field Screening & Surveys'
    },
    {
      profile: anmUsers[0], // Kavita Shinde
      roleLabel: t.roles.anm,
      tag: language === 'hi' ? 'उप-स्वास्थ्य केंद्र' : language === 'mr' ? 'उप-आरोग्य केंद्र' : 'Health Sub-centre',
      icon: Activity,
      desc: language === 'hi' ? 'उप-केंद्र समन्वय नोड' : language === 'mr' ? 'उप-केंद्र समन्वय नोड' : 'Sub-centre Care Coordinator Node'
    },
    {
      profile: adminUsers[0], // Dr. Anand Deshmukh
      roleLabel: t.roles.admin,
      tag: language === 'hi' ? 'नासिक मुख्यालय' : language === 'mr' ? 'नाशिक मुख्यालय' : 'Nashik HQ',
      icon: ShieldCheck,
      desc: language === 'hi' ? 'जिला कमान एवं रेफरल सांख्यिकी' : language === 'mr' ? 'जिल्हा कमान व रेफरल आकडेवारी' : 'District Command & Referral KPIs'
    }
  ];
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col justify-between font-sans text-slate-800">
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-4 sm:px-6 py-6 sm:py-8 flex flex-col justify-center">
            ...
            <div className="relative z-10 p-6 sm:p-8 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
              ...
              <div className="bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/15 w-full lg:w-72 shrink-0">
                <div className="flex items-center space-x-2 text-xs font-bold text-teal-300 uppercase tracking-wider mb-2">
                  ...
                  <span>{language === 'hi' ? 'इंटरैक्टिव सैंडबॉक्स' : language === 'mr' ? 'परस्पर संवादी सँडबॉक्स' : 'Interactive Sandbox'}</span>
                </div>
                <p className="text-[11px] text-slate-200 leading-snug">
                  {language === 'hi'
                    ? 'नीचे किसी भी रोल का चयन करें या लाइव स्वास्थ्य वर्कफ़्लो का परीक्षण करने के लिए डेमो कार्ड पर क्लिक करें।'
                    : language === 'mr'
                    ? 'खालील कोणतीही भूमिका निवडा किंवा थेट आरोग्य प्रणाली अनुभवण्यासाठी डेमो कार्डवर क्लिक करा.'
                ...
                <div className="mt-3 text-[11px] font-bold text-white bg-teal-600/80 rounded-xl px-3 py-1.5 text-center flex items-center justify-center space-x-1.5 shadow-xs">
                  <span>{language === 'hi' ? 'सभी 5 पोर्टल देखें' : language === 'mr' ? 'सर्व 5 पोर्टल्स पहा' : 'Explore All 5 Portals'}</span>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-2.5 pt-2 border-t border-white/10 flex items-center justify-between text-[10px] font-bold text-teal-300 group-hover:text-white">
                      <span>{language === 'hi' ? 'त्वरित लॉगिन' : language === 'mr' ? 'थेट लॉगिन' : 'Instant Login'}</span>
        ...
        {/* Prototype Core Modules / Our Solution */}
        <div className="max-w-5xl mx-auto w-full mb-8 mt-2">
          <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/90 shadow-sm">
            <div className="text-center max-w-xl mx-auto mb-6">
              <span className="text-[11px] font-extrabold text-indigo-700 bg-indigo-50 px-3 py-1 rounded-full border border-indigo-200 uppercase tracking-wider">
                Our Solution
              </span>
              <h3 className="text-xl sm:text-2xl font-black text-slate-900 mt-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {[
                "Assisted Teleconsultation + Facility Coordination",
                "Digital Triage + Appointment & Queue Management",
                "Longitudinal Digital Patient Record + ABDM/ABHA",
                "Closed-Loop Referral + Status Tracking",
                "Diagnostic Availability Module",
                "Medicine & Resource Availability Module",
                "High-Risk Worklist + Reminders + Escalation",
                "Facility Dashboard + Quality Monitoring",
                "Offline-First + Multilingual Voice Assistant",
                "Emergency Priority + Escalation Workflow"
              ].map((feature, idx) => (
                <div key={idx} className="flex items-start space-x-3 p-3 rounded-xl bg-slate-50 border border-slate-100 hover:border-teal-200 transition-colors">
                  <div className="mt-0.5 bg-teal-100 text-teal-700 p-1 rounded-full shrink-0 shadow-2xs">
